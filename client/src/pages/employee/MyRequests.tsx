...
const MyRequests: React.FC = () => {
  ...
  const load = async (p = 1) => {
    try {
      const res = await requestsApi.mine({ page: p, limit: PAGE_SIZE, search: search || undefined, status: status || undefined });
  ...
  return (
    <div style={styles.page}>
      ...
      <table style={styles.table}>
        ...
        <tbody>
          {requests.length === 0 && (
            <tr><td colSpan={5} style={{ textAlign: 'center', padding: '2rem', color: '#94a3b8' }}>No requests found.</td></tr>
          )}
          {requests.map((r) => (
